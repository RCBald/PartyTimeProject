...
def get_event(event_id):
    ...
    sql = "SELECT e.event_id, e.event_name, e.event_date, e.venue_id, v.venue_name, e.start_time, e.end_time, e.invite, e.image_path, e.max_attendees, e.person_name, e.rental_items, e.party_notes FROM Events as e JOIN Venues AS v ON e.venue_id = v.venue_id WHERE e.event_id = %s;"
    conn = get_connection()
    with conn:
        with conn.cursor() as cursor:
...
def add_person(name,address,email,dob,phone,role):
    ...
    sql = "INSERT INTO People(person_name, home_address, email_address, dob, personal_phone, role)VALUES(%s,%s, %s, %s, %s, %s); "
    conn = get_connection()
    with conn:
        with conn.cursor() as cursor:
...
def get_attendees(event_id):
    ...
    sql = "SELECT ea.people_id, p.person_name, p.home_address, p.email_address, p.dob, p.personal_phone FROM Event_attendees AS ea JOIN People AS p ON ea.people_id = p.people_id WHERE ea.event_id = %s;"
    conn = get_connection()
    with conn:
        with conn.cursor() as cursor:
            cursor.execute(sql, (event_id))
...
def get_possible_attendees():
    """returns a list of dictionaries representing nall of the data for people who can attend an event"""   
    sql = "SELECT p.people_id, p.person_name FROM People as p WHERE p.role = 'Attendee';"
    conn = get_connection()
    with conn:
        with conn.cursor() as cursor:
            cursor.execute(sql)
...
def get_host():
    ...
    sql = "SELECT eh.people_id, p.person_name FROM Event_host AS eh JOIN People AS p ON eh.people_id=p.people_id WHERE p.role = 'Host';"
    conn = get_connection()
    with conn:
        with conn.cursor() as cursor:
            cursor.execute(sql)
...
def get_planner():
    ...
    event_id as its primary key"""
    sql = "SELECT DISTINCT ep.people_id, p.person_name FROM Event_planner ep JOIN People as p ON ep.people_id = p.people_id WHERE p.role='Planner';"
    conn = get_connection()
    with conn:
        with conn.cursor() as cursor:
            cursor.execute(sql)
...
def get_venues():
    ...
    sql = "SELECT * FROM Venues ORDER BY venue_id;"
    conn = get_connection()
    with conn:
        with conn.cursor() as cursor:
            cursor.execute(sql)
